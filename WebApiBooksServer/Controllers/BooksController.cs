...
namespace WebApiBooksServer.Controllers
{
    public class BooksController : ApiController
    {
        ...
        public IEnumerable<BooksTable> Get()
        {
            ...
            {
                return entities.BooksTable.ToList();
            }
        }
        // /api/Books/1
        public BooksTable Get(int id)
        {
            using (AzureDBEntities entities = new AzureDBEntities())
            {
                return entities.BooksTable.FirstOrDefault(e => e.ID == id);
            }
        }
        //POST
        ...
        public IHttpActionResult DeleteBook(int id)
        {
            ...
            BooksTable books = db.BooksTable.Find(id);
            if (books == null)
            {
                return NotFound();
            }
            db.BooksTable.Remove(books);
            db.SaveChanges();
            return Ok(books);
        }
        [HttpPut]
        public HttpResponseMessage UpdateBook(int id, [FromBody]BooksTable books)
        {
            try
            {
                ...
                {
                    var dbentity = db.BooksTable.FirstOrDefault(e => e.ID == id);
                    if (dbentity == null)
                    {
